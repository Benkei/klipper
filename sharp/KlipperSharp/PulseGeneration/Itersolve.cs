...
namespace KlipperSharp.PulseGeneration
{
	...
	public class move
	{
		...
		// Populate a 'struct move' with a velocity trapezoid
		public void fill(double print_time
					 , double accel_t, double cruise_t, double decel_t
					 , double start_pos_x, double start_pos_y, double start_pos_z
					 , double axes_d_x, double axes_d_y, double axes_d_z
					 , double start_v, double cruise_v, double accel)
		{
			...
			this.cruise_start_d = accel_t * 0.5 * (cruise_v + start_v);
			this.decel_start_d = this.cruise_start_d + cruise_t * cruise_v;

			// Setup for accel/cruise/decel phases
			this.cruise_v = cruise_v;
			this.accel.c1 = start_v;
			this.accel.c2 = 0.5 * accel;
			this.decel.c1 = cruise_v;
			this.decel.c2 = -this.accel.c2;

			// Setup for move_get_coord()
			this.start_pos.X = start_pos_x;
			this.start_pos.Y = start_pos_y;
			this.start_pos.Z = start_pos_z;
			double inv_move_d = 1.0 / Math.Sqrt(axes_d_x * axes_d_x + axes_d_y * axes_d_y + axes_d_z * axes_d_z);
			this.axes_r.X = axes_d_x * inv_move_d;
			this.axes_r.Y = axes_d_y * inv_move_d;
		...
		{
			if (move_time < accel_t)
				// Acceleration phase of move
				return accel.eval_accel(move_time);
			move_time -= accel_t;
			if (move_time <= cruise_t)
				// Cruising phase
				return cruise_start_d + cruise_v * move_time;
			// Deceleration phase
			move_time -= cruise_t;
			return decel_start_d + decel.eval_accel(move_time);
		}

		// Return the XYZ coordinates given a time in a move
		public Vector3d get_coord(double move_time)
		{
			double move_dist = get_distance(move_time);
			return new Vector3d(
				start_pos.X + axes_r.X * move_dist,
				start_pos.Y + axes_r.Y * move_dist,
				start_pos.Z + axes_r.Z * move_dist
			);
		}
